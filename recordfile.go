...
func New(st interface{}) (*RecordFile, error) {
	...
	for i := 0; i < typeRecord.NumField(); i++ {
		...
		case reflect.Map:
		default:
			return nil, fmt.Errorf("invalid type %d, %v %s", i,
				f.Name, kind)
		}
...
func (rf *RecordFile) Read(name string, beginLine int) error {
	...
	// make records
	records := make([]interface{}, len(lines)-beginLine)
	...
	for n := beginLine; n < len(lines); n++ {
		value := reflect.New(typeRecord)
		records[n-beginLine] = value.Interface()
		record := value.Elem()
		line := lines[n]
		...
		for i := 0; i < typeRecord.NumField(); i++ {
			...
			if f.Tag == "index" {
				...
				index[field.Interface()] = records[n-beginLine]
			}
		}
	}
